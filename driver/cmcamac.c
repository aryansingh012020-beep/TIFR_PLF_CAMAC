 ...
 *
 */
// #include <linux/config.h>  //Commented by A.C.
...
/* Compatibility shims: err() and info() were removed from kernel headers ~2.6.33 */
#define err(fmt, ...)  pr_err(fmt "\n", ##__VA_ARGS__)
#define info(fmt, ...) pr_info(fmt "\n", ##__VA_ARGS__)

#define CMCSLEEP	(8*HZ)
#define CMCTMO		(2*HZ)
#define CMCBLMPLY	8
/*	Our endpoints		*/	
...
static DEFINE_MUTEX(cmcamac_table_mutex);	/* replaces removed BKL */	
...
{
	int retval = 0;
	int actual;	/* usb_bulk_msg requires int *, not size_t * */
	...
	return count;	/* original return semantics preserved */
}
...
{
	int retval = 0;
	int cnt;	/* usb_bulk_msg requires int *, not size_t * */
...
static int cmcamac_getnumber(struct usb_cmcamac *dev)
{
	...
	/* CAMAC data words are always 32-bit; use __le32 to be explicit */
	sndbuf = kmalloc_array(4, sizeof(__le32), GFP_KERNEL);
	if (!sndbuf) return -ENOMEM;
	sndbuf[0] = cpu_to_le32(0xFFFFFFFF);
	sndbuf[1] = cpu_to_le32(0);
	sndbuf[2] = cpu_to_le32(0x3C11);
	sndbuf[3] = cpu_to_le32(0x0E000000);

	buf = kmalloc_array(128, sizeof(__le32), GFP_KERNEL);
	if (!buf) {
		kfree(sndbuf);
		return -ENOMEM;
	}
...
/*		Send something with PKTEND		*/
	retval = cmcamac_send(dev, sndbuf, 4 * sizeof(__le32));
	if (retval < 0) goto ERR_RET;
	if (retval != 4 * sizeof(__le32)) {
...
/*		Try to read until timeout - this is the
	...
	for (i=0, ecnt=0; i<MAXREADS; i++) {
		retval = cmcamac_receive(dev, buf, 128 * sizeof(__le32));
...
static int cmcamac_open(struct inode *inode, struct file *file)
{
	...
	/* Bounds check before indexing minor table */
	if (subminor >= CMCAMAC_MAX_MINORS) {
		retval = -ENODEV;
		goto exit;
	}

	mutex_lock(&cmcamac_table_mutex);
	switch(cmcamac_minor_table[subminor]) {
	case CMC_NONE:
		mutex_unlock(&cmcamac_table_mutex);
		retval = -ENODEV;
		break;
	case CMC_BUSY:
		mutex_unlock(&cmcamac_table_mutex);
		err("Only one process can use CMCAMAC crate #%d.", subminor);
		retval = -EBUSY;
		break;
	case CMC_READY:
		cmcamac_minor_table[subminor] = CMC_BUSY;
		mutex_unlock(&cmcamac_table_mutex);
		break;
	}
	...
	if (!interface) {
		...
		cmcamac_minor_table[subminor] = CMC_NONE;
		retval = -ENODEV;
		goto exit;
	}
	...
	/* On modern kernels interface->minor == crate_number directly */
	if (i != subminor) {
		err("Crate number unexpectedly changed from %d to %d", subminor, i);
		cmcamac_minor_table[subminor] = CMC_NONE;
		retval = -ENODEV;
		goto exit;
	}
...
static int cmcamac_release(struct inode *inode, struct file *file)
{
	...
	if (subminor < CMCAMAC_MAX_MINORS &&
	    cmcamac_minor_table[subminor] == CMC_BUSY)
	    cmcamac_minor_table[subminor] = CMC_READY;
	mutex_unlock(&cmcamac_table_mutex);
...
static ssize_t cmcamac_read(struct file *file, char *buffer, size_t count, loff_t *ppos)
{
	...
	int retval = 0;
	size_t rcnt;
	int l;	/* usb_bulk_msg requires int *, not size_t * */
	int i;
	...
	for (i=0;i<MAXREADS;i++) {
		...
		if (count > rcnt && l > 0 &&  
			copy_to_user(&buffer[rcnt], dev->bulk_in_buffer, min_t(size_t, l, count - rcnt))) 
				return -EFAULT;
		rcnt += l;
...
static ssize_t cmcamac_write(struct file *file, const char *user_buffer, size_t count, loff_t *ppos)
{
	...
	for (wcnt = 0; wcnt < count; wcnt += l) {
		l = (int)min_t(size_t, dev->bulk_out_size, count - wcnt);
...
static long cmcamac_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case CMC_CTL_RESET :
		...
		data = kmalloc(sizeof(__le32), GFP_KERNEL);
...
static struct file_operations cmcamac_fops = {
	...
	.unlocked_ioctl = cmcamac_ioctl,	/* .ioctl removed in 2.6.36 */
};
...
static struct usb_class_driver cmcamac_class = {
	.name =		NULL,
	.fops =		&cmcamac_fops,
	//.mode =		S_IFCHR | S_IRUSR | S_IWUSR | S_IRGRP | S_IWGRP | S_IROTH | S_IWOTH, //Commented vy A.C.
	.minor_base =	-1,
};
...
static int cmcamac_probe(struct usb_interface *interface, const struct usb_device_id *id)
{
	...
	int i;
	int retval = -ENOMEM;
	static char name[32];	/* BUS_ID_SIZE removed in 2.6.30; static avoids dangling ptr */
...
static void cmcamac_disconnect(struct usb_interface *interface)
{
	...
	/* cmcamac_class.name still points to static buffer set in probe() */
	usb_deregister_dev(interface, &cmcamac_class);
	/* On modern kernels interface->minor == crate_number directly */
	if (minor >= 0 && minor < CMCAMAC_MAX_MINORS)
		cmcamac_minor_table[minor] = CMC_NONE;
	...
	info("USB CMCAMAC #%d now disconnected", minor);
}

static struct usb_driver cmcamac_driver = {
	//.owner =	THIS_MODULE, //Commented by A.C.
	.name =		"cmcamac",
	.probe =	cmcamac_probe,
